class TopicsController < ApplicationController
  ...
  def index
    @topics = Topic.all
    @recent_posts = Post.includes([:users_posts, :image_attachment => :blob], :posts_tags, :tags, :comments).references(:users_posts).first(3)
  end
  ...
  def destroy
    ...
    respond_to do |format|
      ...
      format.json { render json: "Topic Deleted" }
    end
  end
